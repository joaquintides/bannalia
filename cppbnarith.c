/* Cpp bignum arithmetic operations.
 *
...
#define BN_MUL_OP_IMPL(x,shift,bn2,result) \
(shift(0)) \
(bn2) \
(BN_ADD(result,BN_DIGIT_MUL(bn2,x)BOOST_PP_SEQ_TAIL(shift)))

#define BN_MUL_STATE_SHIFT(state) BOOST_PP_SEQ_ELEM(0,state)
#define BN_MUL_STATE_BN2(state) BOOST_PP_SEQ_ELEM(1,state)
#define BN_MUL_STATE_RESULT(state) BOOST_PP_SEQ_ELEM(2,state)
